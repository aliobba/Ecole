...
public class ConsulterCahierEnfant extends Fragment {

    ArrayList<Exercice> exercices;
    ArrayList<Matiere> matieres;
    AdapterConsultCahierEnfant adapter;
    SwipeRefreshLayout refresh;
    RecyclerView rv_liste;
    String url2 = "http://"+ UserLoged.url+"/findCahier/";
    String url = "http://"+ UserLoged.url+"/findAllExerciceEleve/";

    public ConsulterCahierEnfant() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View root = inflater.inflate(R.layout.activity_consulter_cahier_enfant, container, false);
        ...
        adapter = new AdapterConsultCahierEnfant(getContext(),exercices,matieres);
        rv_liste.setAdapter(adapter);

        refresh.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            @Override
            public void onRefresh() {
                GetExerciceReq(UserLoged.id_cahier);
    ...
    private void GetExerciceReq(int idcahier){
        RequestQueue queue= Volley.newRequestQueue(getContext());

        exercices.clear();
        matieres.clear();
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url+idcahier, new Response.Listener<String>() {
            @Override
            public void onResponse(String response){
                try {
